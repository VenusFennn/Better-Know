...
默认假设读者理解产品和业务，但不熟悉当前项目代码库。方案提及关键函数、类、组件、Hook、Store、Controller、Service、Repository、字段、文件、接口、表、Topic、缩写或专业术语时，会在第一次出现的相邻位置解释：

1. 它是什么类型的对象；
2. 它负责什么；
3. 为什么与当前问题或方案有关。

表达遵循“业务含义在前，技术标识在后”，例如：

```text
负责获取文件夹项目并更新页面状态的 `refreshFolderProjects` 函数
```

原始技术名称仍然保留，便于实施者搜索代码；同一对象后续使用稳定中文短称，不重复解释。多个关键对象集中出现时，方案会在当前相关章节动态生成“关键对象说明”表，而不是增加固定第八章节或整库术语词典。

解释必须来自源码、调用关系、测试、文档、契约或运行证据。证据不足时会明确标记为推断，不会根据名称编造职责。
